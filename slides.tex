...
\begin{frame}[plain]
  ...
  Comme le titre de ma thèse l'indique, je vais vous parler aujourd'hui de dynamiques épigénomiques, de thymopoièse et reproductibilité. Contrairmeent à ce qu'indique le titre de ma thèse, je ne vais pas vous parler de spermatogénèse aujourd'hui afin de pouvoir aborder les autres sujets avec plus de détails.
...
\begin{frame}[plain]
  ...
  \note{Blueprint est l'un de ces consortiums auxquels ont participé les Dr Spicuglia et Asnafi afin de fournir les épigénomes des précurseurs thymiques humains. Aujourd'hui je vais vous présenter l'analyse réalisée pendant ma thèse à partir de ces données.
...
\begin{frame}{The thymus, the organ of thymopoiesis}
  ...
  \includegraphics[width=\textwidth, height=\textheight, keepaspectratio]{\smi}%
  \note{La thymopoièse a lieu dans le thymus, représenté ici en rouge vif.}%
\end{frame}
\begin{frame}{The thymus, the organ of thymopoiesis}
  %\begin{frame}{The thymus, a critical organ for functional adaptive immune system}
  ...
  \begin{frame}{The thymus, the organ of thymopoiesis}
    ...
    \begin{frame}{Nomenclature for major T cell differentiation stages}
      ...
      \includegraphics[width=\textwidth, height=.95\textheight, keepaspectratio]{\smi}%
      \note{Dans le cadre de notre étude, nous avons accès à ces différents stades sur la base des marqueurs de surface utilisé pour le tri cellulaire. Comme les noms ne correspondent pas forcément avec la nomenclature principale de la littérature, vous avez l'équivalence entre les deux nomenclatures avec le code couleur.}%
    \end{frame}
    \begin{frame}{Available experimental approaches}
      \def\smi{out/ln/updir/mw-gcthesis-oral/ink/thymus/6.pdf}
      \includegraphics[width=\textwidth, height=.95\textheight, keepaspectratio]{\smi}%
      \note{Sur ces échantillons, nous avons accès aux ChIP-Seq des six marques d'histones jugées principales par les grands consortiums; ainsi que du RNA-seq, du whole-genome bisulfite seq, et de l'atac-seq pourles stades les plus précoces. Tout ceci à l'exception du stade ISP pour lequel nous n'avons accès qu'au transcriptome.}%
    \end{frame}
    \begin{frame}{Objectives}
      ...
      \end{itemize}
      \note{A partir de ces données, nous avons deux objectifs principaux: premièrement de replacer les populations thymiques dans le paysage épigénomique de l'hématopoièse, principalement afin de fournir la preuve de la pertinence des données générées et leur utilité pour l'étude par la communauté scientifique de mécanismes épigénomiques et des réseaux de régulations en jeu lors de la différentation des lymphocytes T.

      Deuxièmement, de procéder nous même à une étude de dynamique épigénomique avec une attention particulière portée aux éléments régulateurs distaux et aux enhanceurs.
      }%
    \end{frame}
