...
\begin{frame}{Thesis summary}
  \def\smi{out/ln/updir/mw-gcthesis-oral/ink/thesis-mind-map-epigenomic-dynamics.pdf}
  \includegraphics[width=\textwidth, height=.85\textheight, keepaspectratio]{\smi}%
  \note{Au cours de celle-ci, je me suis intéressé à l'étude de dynamiques épigénomiques dans deux modèles biologiques.}%
\end{frame}
\begin{frame}{Thesis summary}
  \def\smi{out/ln/updir/mw-gcthesis-oral/ink/thesis-mind-map-publications.pdf}
  \includegraphics[width=\textwidth, height=.85\textheight, keepaspectratio]{\smi}%
  \note{Lors de mes deux premières années, j'ai eu l'opportunité de participer à des travaux sur le modèle de la spermatogénèse murine. Ceux-ci  étaient dirigés par Saadi Khochbin et Sophie Rousseaux à Grenoble et ont abouti à la publication de trois articles.

J'ai ensuite travaillé sur le modèle de la thymopopïèse humaine au cours des 20 derniers mois. J'ai choisi de vous présenter aujourd'hui dans un premier temps ce dernier travail car il n'est pas encore publié.
  }%
\end{frame}
\begin{frame}{Thesis summary}
  \def\smi{out/ln/updir/mw-gcthesis-oral/ink/thesis-mind-map.pdf}
  \includegraphics[width=\textwidth, height=.85\textheight, keepaspectratio]{\smi}%
  \note{J'aborderai dans un deuxième temps les problématiques méthodologiques rencontrées en tant que bioinformaticien pour l'analyse des données issues de séquençage à haut débit.}%
\end{frame}
...
\begin{frame}{One genome, thousands phenotypes}
  ...
  \note{La majorité des cellules d'un organisme partagent le même génome. Pourtant, chacun des types cellulaires exprime un programme génétique différent lui permettant d'assurer la spécificité de sa fonction. Ceci est illustré ici avec les types cellulaires du système hématopoiétique.
  Il existe plusieurs niveaux de régulation à l'oeuvre pour permettre l'activation et l'inactivation de ces programmes génétiques et c'est ce que je vais présenter maintenant.}%
...
\begin{frame}{Genome Complexity: From structural organization...}
  ...
  \includegraphics[width=1\textwidth, height=0.75\textheight, keepaspectratio]{\smi}%
  \blfootnote{Adapted from \textit{Dogan and Liu, 2018, Nature Plants}}
  \note{Ces fréquences d'interaction plus importantes pour certains sites sont la conséquence du repliement de l'ADN en boucles, boucles qui permettent le rapprochement spatial et l'interaction de régions régulatrices pourtant éloignées dans la séquence nucléotidique. 
  %, repliement qui est contrôlé par la présence de complexes protéiques, représentés ici en violet.}% comprenant généralement Polycomb et/ou CTCF.}
...
%\end{frame}
\begin{frame}{Genome Complexity: From structural organization to epigenetic regulation}
  \def\smi{out/ln/updir/mw-gcthesis-oral/ink/epigenetic-of-regulatory-elements-no-classif-heterochromatin.pdf}
  \includegraphics[width=1\textwidth, height=0.75\textheight, keepaspectratio]{\smi}%
...
\end{frame}
\begin{frame}{Genome Complexity: From structural organization to epigenetic regulation}
  \def\smi{out/ln/updir/mw-gcthesis-oral/ink/epigenetic-of-regulatory-elements-heterochromatin.pdf}
  \includegraphics[width=1\textwidth, height=0.75\textheight, keepaspectratio]{\smi}%
  \note{La connaissance de la présence de différentes combinaisons de ces modifications épigénétiques permet de classifier l'ensemble du génome en éléments fonctionnels tels que les promoteurs et enhancers.
  %, actifs ou inactifs.
  %Pour le projet dont je vais vous parler, nous avons accès à six modifications d'histones qui concernent toutes des lysines de l'histone 3. 
  Notamment, la présence combinée de méthylation sur la lysine 4 et d'acétylation sur la lysine 27 permet de détecter des enhancers actifs.
  Il existe beaucoup de modifications épigénétiques et ici ne sont représentées que les principales qui seront évoquées aujourd'hui.
...
\begin{frame}{Seeing the big epigenomic picture by mapping blood cells}
  ...
  \begin{frame}{The thymus, the organ of thymopoiesis}
    ...
    \includegraphics[width=\textwidth, height=\textheight, keepaspectratio]{\smi}%
    \blfootnote{Diagram adapted from \textit{\href{https://commons.wikimedia.org/wiki/File:Diagram_showing_the_position_of_the_thymus_gland_CRUK_362.svg}{Cancer UK Research / Wikimedia Commons}}}
  ...
  \begin{frame}{The thymus, the organ of thymopoiesis}
    ...
    \includegraphics[width=\textwidth, height=\textheight, keepaspectratio]{\smi}%
    \blfootnote{H\&E stained section of human infant thymus taken from \textit{\href{https://www.dartmouth.edu/~anatomy/Histo/lab_6/lymphoid/DMS117/popup.html}{Darthmouth College}}}
  ...
  \begin{frame}{The thymus, the organ of thymopoiesis}
    ...
    \includegraphics[width=\textwidth, height=.95\textheight, keepaspectratio]{\smi}%
    \blfootnote{Diagram taken from \textit{\href{https://www.dartmouth.edu/~anatomy/Histo/lab_6/lymphoid/DMS117/popup.html}{Rothenberg et al., 2008, Nature Reviews Immunology}}}
  ...
  \begin{frame}{Biological objectives}
    ...
    Il s'agit ensuite de procéder nous même à une de ces études de dynamique épigénomique, avec une attention particulière portée aux éléments régulateurs distaux et aux enhanceurs, afin d'identifier de nouveaux mécanismes de régulation épigénétique.
    }%
  \end{frame}
  ...
  \begin{frame}{Chromatin segmentation meets expected epigenetics status for known cell-type specific genes}
    ...
    \includegraphics[width=\textwidth, height=.95\textheight, keepaspectratio]{\smi}%
    \note{Dans l'ensemble les données épigénomiques produites capturent avec précision les identités biologiques des stades thymiques, et ici vous avez une illustration de cela avec quelques genes connus pour être spécifique de certains stades de différentiation thymique.
    %, on peut en se concentrant sur les états en jaune, rapidement se rendre compte que l'épigénome constitué des six marques d'histones capture avec précision les identités biologiques des stades thymiques.

    A partir de ces données validées, Nous allons maintenant chercher à caractériser des dynamiques épigénomiques de régions distales.}
  ...
  \begin{frame}{Differential expression analysis highlights genes associated with a dynamic putative enhancer}
    ...
    \includegraphics[width=\textwidth, height=.85\textheight, keepaspectratio]{\smi}%
    \note{Et parmi les gènes uprégulés, celui qui possède le ratio d'enrichissement le plus élevé entre les deux stades est le gène du récepteur alpha des lymphocytes T.

    Pour finir cette analyse épigénomique, on va s'intéresser un peu plus en détail au locus de ce gène.}%
  ...
  \begin{frame}{Epigenetic dynamics of TCRA locus}
    ...
    \note{Le TRAC obtenu en analyse différentielle correspond en fait à la partie constante du gène codant pour le TCRalpha. L'enhancer alpha est le nom de l'enhancer connu du locus et qui a été notamment étudié par Salva dans le cadre de sa thèse. 

    Il correspond à la région distale qui ressort associée au TCRA. Un zoom sur cette région se trouve à droite. On vérifie qu'il est constitutivement déméthylée et ouvert au cours de la différentiation. Et on y observe une H3K27acétylation qui augmente fortement entre les stades tCD34 et early cortical.
  ...
  \begin{frame}{\emph{TLX homeodomain oncogenes mediate} T cell maturation arrest in T-ALL via interaction with ETS1 and \emph{suppression of TCR\textalpha{} gene expression}}
    ...
    \note{L'équipe du professeur Asnafi avait mis en évidence en 2012 dans certaines leucémies l'action des facteurs TLX1 et TLX3 comme bloqueur de l'activation de l'enhancer alpha ce qui entraine la répression du gène. Ces gènes à homéodomaine TLX1 et 3 feraient de bons candidats pour expliquer le même mécanisme au cours de la différentiation saine des lymphocytes T, si seulement ils étaient exprimés. Comme ce n'est pas le cas, ...}%
  \end{frame}
  ...
  \begin{frame}{Mature T-lymphoblastic leukemias provide another evidence for Homeobox family genes repressive action on E\textalpha{}}
    \def\smi{out/ln/updir/mw-gcthesis-oral/ink/tall/ealpha_h3k27ac_families.pdf}
    \includegraphics[width=\textwidth, height=.8\textheight, keepaspectratio]{\smi}%
    \note{Pour tester cette hypothèse, on peut utiliser des ChIP-Seq H3K27ac produits pour différentes leucémies T lymphoblastiques matures. Matures signifie ici que les leucémies sont dérivés de stade tardif de la différentiation T et que, par conséquent, l'expression des gènes HOXA, si elle est observée, est purement oncogénique. On sépare les leucémies selon leur type, HOXA positive, TLX positive ou Homeodomain negative. Et on représente le signal normalisé de H3K27ac dans l'enhancer alpha sous la forme de violin-box-dot-plot pour observer une différence significative marqué entre le groupe n'exprimant pas de gène à homéodomaine et ceux de gènes en en exprimant. Ceci constitue la première évidence de l'action répressive des HOXA sur l'enhancer alpha,...
    }%
  \end{frame}
%  \begin{frame}{Mature T-lymphoblastic leukemias provide another evidence for Homeobox family genes repressive action on E\textalpha{}}
...
%  \end{frame}
  ...
  \begin{frame}{Main requirements for \emph{methods} reproducibility}
    ...
    \note{Les exigences principales pour atteindre cette reproductibilité de méthode concernent des propriétés des données, outils et code d'analyses utilisés pour obtenir un résultat. Plus précisément, les données doivent être trouvables, accessibles, interopérables et réutilisables. 
    %Ces concepts sont détaillés par la charte des principes du FAIR data.
    Les logiciels doivent être portables et le code d'analyse doit être traçable et automatisé.}%
  \end{frame}
  ...
  \begin{frame}[fragile]{Elements of solution for reproducibility in bioinformatics}
    ...
    La définition de la reproductibilité s'arrête à ce qui vient d'être décrit mais elle peut se prolonger dans l'esprit de ceux qui l'emploient dans des scénarios qui s'étendent au delà de sa définition initiale.
    }%
  \end{frame}
  ...
  \begin{frame}{Reuse scenario}
    ...
    \includegraphics[width=\textwidth, height=.95\textheight, keepaspectratio]{\smi}%
    \note{Par exemple dans un scénario de réutilisation de l'analyse à d'autres données.}%
  \end{frame}
  ...
  %  }
  %\end{frame}
  \begin{frame}{Integrating workflows using subworkflows...}
    \def\smi{out/ln/updir/mw-gcthesis-oral/ink/snakemake/ABCDE-separated.pdf}
    \includegraphics[width=\textwidth, height=.75\textheight, keepaspectratio]{\smi}%
    \note{La question est principalement de décider comment intégrer les différentes analyses. Une première approche consiste à considérer les workflows de chacune des analyses indépendemment, comme des sous-workflows de l'analyse intégrative. 

    Ici deux workflows ont été développés indépendemment, l'un permettant de traiter un type d'échantillons selons les tâches A, B et C. l'autre permettant de traiter un autre type d'échantillons selon les tâches D et E.}%
  \end{frame}
  \begin{frame}{Integrating workflows using subworkflows may leads to poor parallelization}
    \def\smi{out/ln/updir/mw-gcthesis-oral/ink/snakemake/ABCDE-dotted.pdf}
    \includegraphics[width=\textwidth, height=.75\textheight, keepaspectratio]{\smi}%
    \note{Il existe une subtilité cependant puisque la tâche D a besoin de fichiers produits par la tâche B pour être réalisée. Le problème qui apparait si on considère les deux workflows comme des sous-workflows est qu'il va falloir attendre la fin de la tâche C avant que la tâche D puisse commmencer.}%
  \end{frame}

  \begin{frame}{Integrating workflows using merged workflow lead to optimal parallelization}
    \def\smi{out/ln/updir/mw-gcthesis-oral/ink/snakemake/ABCDE.pdf}
    \includegraphics[width=\textwidth, height=.75\textheight, keepaspectratio]{\smi}%
    \note{En fusionnant les workflows ensemble, la tâche D pourra démarrer dès la fin de la tâche B, d'où un gain de temps d'attente.}%
  \end{frame}
  \begin{frame}{Integrating workflows using merged workflow allows to reduce code redundancy}
    \def\smi{out/ln/updir/mw-gcthesis-oral/ink/snakemake/ABCDCprime.pdf}
    \includegraphics[width=\textwidth, height=.75\textheight, keepaspectratio]{\smi}%
    \note{Une autre subtilité pourrait intervenir. La tâche E n'est en fait qu'une variante très similaire de la tâche C qui repose sur le même outil mais avec de légères variations comme par exemple le fait que l'une accepte en entrées des données produites par B et l'autre par D.

    Pour le développeur, il est plus facile de maintenir du code si celui-ci ne contient pas de redondance. Une raison est que si l'on décide d'amliorer la règle C, il faut non pas appliquer les modifications sur la règle C mais aussi sur la règle C' et toutes les autres variantes qui pourraient exister.
    }%
  \end{frame}
  \begin{frame}{Integrating workflows using merged workflow allows to reduce code redundancy}
    \def\smi{out/ln/updir/mw-gcthesis-oral/ink/snakemake/ABCDC.pdf}
    \includegraphics[width=\textwidth, height=.75\textheight, keepaspectratio]{\smi}%
    \note{Si on fusionne correctement les workflows, on peut aboutir à ce résultat qui simplifie la base de code nécessaire pour réaliser les analyses.}%
  \end{frame}
  ...
  \begin{frame}{Metaworkflow defines coding conventions for modular workflow development, multiple collaborations and users}
    ...
    \resizebox{!}{.8\textheight}{%
      ...
      \begin{frame}{Acknowledgments}
        \centering
        \def\smi{out/r/gcthesis_acknowledgments/1.pdf}
        \includegraphics[width=\textwidth, height=.48\textheight, keepaspectratio]{\smi}
        \def\smi{out/r/gcthesis_acknowledgments/2.pdf}
        \includegraphics[width=\textwidth, height=.48\textheight, keepaspectratio]{\smi}
        \note{
